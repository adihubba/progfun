...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def timesAcc(chars: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = {
      if (chars.isEmpty)
        ...
        val head = chars.head
        val tail = chars.tail
        timesAcc(tail.filter { _ != head }, (head, 1 + tail.count { _ == head }) :: acc)
      }
    }
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    ...
    else freqs.map(pair => Leaf(pair._1, pair._2)).sortBy { _.weight }
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    if (trees.isEmpty || singleton(trees))
      ...
      (fork :: trees.tail.tail).sortBy { weight(_) }
    }
  }
   ...
   */
  def until(condition: List[CodeTree] => Boolean, reducer: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
    if (condition(trees))
      trees
    else {
      until(condition, reducer)(reducer(trees))
    }
  }
  ...
  def encodeChar(tree: CodeTree, Text: Char): List[Bit] = {
    def encodeCharAcc(tree: CodeTree, acc: List[Bit]): List[Bit] = {
      tree match {
        case Leaf(Text, _)                                 => acc
        case fork: Fork if chars(fork.left) contains Text  => encodeCharAcc(fork.left, acc :+ 0)
        case fork: Fork if chars(fork.right) contains Text => encodeCharAcc(fork.right, acc :+ 1)
      }
    }
    encodeCharAcc(tree, Nil)
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
    def convertAcc(subTree: CodeTree, code: List[Bit], acc: CodeTable): CodeTable = {
      subTree match {
        case Leaf(c, _)              => acc :+ (c, code)
        case Fork(left, right, _, _) => mergeCodeTables(convertAcc(left, code :+ 0, acc), convertAcc(right, code :+ 1, acc))
      }
    }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    text.flatMap { codeBits(codingTable)(_) }
  }
}
